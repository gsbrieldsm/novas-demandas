      ...
      <div className="max-w-4xl mx-auto w-full px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-1">{rows.filter(r => r.pag?.recebido).length} de {clientesAtivos.length} clientes</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Pendente</p>
            <p className={clsx('text-2xl font-bold', totalPendente > 0 ? 'text-red-500' : 'text-slate-400')}>{formatBRL(totalPendente)}</p>
            <p className="text-xs text-slate-400 mt-1">{rows.filter(r => !r.pag?.recebido).length} clientes aguardando</p>
          </div>
        </div>
        ...
        {avulsosMes.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-slate-700">Avulsos Concluídos no Mês</h2>
              <span className="text-sm font-bold text-green-600">{formatBRL(totalAvulso)}</span>
            </div>
            <table className="w-full">
              <thead>
                <tr className="text-left">
                  ...
                  <th className="px-6 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Cliente</th>
                  <th className="px-6 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Valor</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {avulsosMes.map(t => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className="text-sm font-semibold text-green-600">{formatBRL(t.budget_value!)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
